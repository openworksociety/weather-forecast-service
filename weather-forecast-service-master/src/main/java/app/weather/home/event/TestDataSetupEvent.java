...
public class TestDataSetupEvent implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		locationInfoRepository.save(
				LocationInfo.builder().city("Mumbai").country("India").latitude(50.551086).longitude(8.993682).build());

		dailyWeatherInfoRepository.save(DailyWeatherInfo.builder().city("Mumbai").date(LocalDate.now()).humidity(30)
		...
		for(int i =1;i<=24;i++) {
			hourlyWeatherInfoRepository.save(HourlyWeatherInfo.builder().city("Mumbai").date(LocalDate.now()).hourCount(i)
					.humidity(35).temperature(25).wind("10 Km/h").build());
			hourlyWeatherInfoRepository.save(HourlyWeatherInfo.builder().city("Hamburg").date(LocalDate.now()).hourCount(i)
					.humidity(35).temperature(25).wind("10 Km/h").build());
		}
